...
无需 API Key，不依赖云服务，不上传你的代码。它优先本地运行，基于规则扫描项目结构并生成 README 草稿。
...
| 数据库 | MySQL、PostgreSQL、Redis、MongoDB、向量数据库 |
| 测试 | pytest、Jest、Vitest、Playwright、JUnit |
| 部署线索 | `Dockerfile`、`docker-compose.yml`、`.env.example`、Nginx、Vercel |
...
本仓库包含一个实验性的 MCP 风格 stdio 工具服务，方便 Agent 调用。

```bash
repo-readme-polisher-mcp
```

请求示例见 [docs/MCP.md](docs/MCP.md)。

## Release 工作流

推送版本 tag 会触发 GitHub Actions 自动创建 Release：
...
- [x] 中英文 README 草稿生成
- [x] 可选 AI 改写提示词模式：`--ai`
- [x] 实验性 MCP-style Agent 集成
- [x] GitHub Release 工作流
- [ ] 更丰富的框架识别
- [ ] Markdown 质量评分
- [ ] 可配置 README 模板
- [ ] GitHub 仓库元数据支持
